...
import co.com.claro.financialintegrator.thread.ReporteCreditosFinalizados;
import co.com.claro.financialintegrator.util.UidService;

public class NotificacionCreditosFinalizados extends GenericProccess {
	...
	private Boolean executeProd(String uid) {
		...
		if (databaseSourceMotorBloqueo != null
			...
			try {
				...
				logger.info("CONSULTAR_CREDITOS_FINAL_GB -> P_EXITO : "
						+ P_EXITO);
				if (rs != null) {
					try {
						...
						while (rs.next()) {
							...
							//
							BigDecimal ID_CREDITOS_FINAL_GB = rs
									.getBigDecimal("ID_CREDITOS_FINAL_GB");
							element.put("ID_CREDITOS_FINAL_GB",
									ID_CREDITOS_FINAL_GB);
							//
							...
							//
							java.sql.Date FECHA = rs
									.getDate("FECHA_FINAL_CREDITO");
							element.put("FECHA_FINAL_CREDITO", NRO_PRODUCTO);
							//
							String detalle = null;
							// Se revisan conexiones
							...
							element.put("REFERENCIA_PAGO", detalle);
							logger.info("**** Create Thread : " + detalle);
							ReporteCreditosFinalizados thread = new ReporteCreditosFinalizados(
									ID_CREDITOS_FINAL_GB, IMEI,
									NRO_PRODUCTO, FECHA, detalle,
	...
	public void process() {
		...
		logger.info(".. RUNNING NOTIFICACION CREDITOS FINALIZADOS 1.0 ..");
