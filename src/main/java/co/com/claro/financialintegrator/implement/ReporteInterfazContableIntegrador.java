...
public class ReporteInterfazContableIntegrador extends GenericProccess {

	private Logger logger = Logger.getLogger(ReporteInterfazContableIntegrador.class);

	@Override
	public void process() {
		logger.info(" -- PROCESANDO REPORTE INTERFACE CONTABLE INTEGRADOR --");
	...
	private void procesarReporte(String path, String path_process, String uid) {
		List<String> reporte = this.consultarInterfazContableIntegrador(uid);
	...
	private List<String> consultarInterfazContableIntegrador(String uid) {
		...
		String pExito = null;
		List<String> listaInterfazContableIntegrador = new ArrayList<String>();

		try {
			dataSource = this.getPros().getProperty("DatabaseDataSource").trim();
			callReporteDataCredito = this.getPros().getProperty("callConsultaInterfazContableIntegrador");
			...
			if (pExito.equals("TRUE")) {
				...
				while (rsInfClientesMovil.next()) {
					...
					listaInterfazContableIntegrador.add(lineString);
				}
			}

			String observacion = "Ejecutada correctamente consulta de reporte Interfaz Contable Integrador";
			String NombreProceso = "ReporteInterfazContableIntegrador";
			registrar_auditoriaV2(NombreProceso, observacion, uid);
		} catch (Exception e) {
			String observacion = "Error consulta de reporte Interfaz Contable Integrador" + e.getMessage();
			String NombreProceso = "ReporteInterfazContableIntegrador";
			...
			try {
				...
				logger.error("Error cerrando consulta de reporte Interfaz Contable Integrador" + e.getMessage());
			}
		}
		return listaInterfazContableIntegrador;
	}
	...
	private boolean crearArchivo(List<String> reporte) {
		...
		try {
			...
			logger.error("Error Creando Reporte Interfaz Contable Integrador" + ex.getMessage(), ex);
		} finally {
	...
	/**
	 * Configuración de reporte: Interfaz Contable Integrador contienen elcuerpo del
	 * reporte
	 * 
	 * @param file
	 * @return
	...
	private FileConfiguration configurationFileReporte() {
		...
		type.setLength(8);
		type.setSeparator("\t");
		type.setName("fechaContabilizacion");
		type.setTypeData(new ObjectType(String.class.getName(), ""));
		type.setStringcomplement(" ");
		type.setLeftOrientation(false);
		_types.add(type);
		...
		type.setLength(2);
		type.setSeparator("\t");
		type.setName("claveContabilizacion");
		type.setTypeData(new ObjectType(String.class.getName(), ""));
		...
		type.setLength(2);
		type.setSeparator("\t");
		type.setName("importeMonedaLocal");
		type.setTypeData(new ObjectType(String.class.getName(), ""));
		type.setStringcomplement(" ");
		type.setLeftOrientation(false);
		_types.add(type);
		...
		type.setLength(1);
		type.setSeparator("\t");
		type.setName("indicadorImpuestos");
		type.setTypeData(new ObjectType(String.class.getName(), ""));
		type.setStringcomplement(" ");
		type.setLeftOrientation(false);
		_types.add(type);
		...
		type.setLength(3);
		type.setSeparator("\t");
		type.setName("estratoCanalDistrib");
		type.setTypeData(new ObjectType(String.class.getName(), ""));
		type.setStringcomplement(" ");
		type.setLeftOrientation(false);
		_types.add(type);
		...
		type.setLength(3);
		type.setSeparator("\t");
		type.setName("divisionGrVendedores");
		type.setTypeData(new ObjectType(String.class.getName(), ""));
		type.setStringcomplement(" ");
		type.setLeftOrientation(false);
		_types.add(type);
		...
		type.setLength(3);
		type.setSeparator("\t");
		type.setName("paqueteGrArticulos");
		type.setTypeData(new ObjectType(String.class.getName(), ""));
		type.setStringcomplement(" ");
		type.setLeftOrientation(false);
		_types.add(type);
