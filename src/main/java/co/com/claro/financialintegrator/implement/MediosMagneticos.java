...
public class MediosMagneticos extends GenericProccess {
	private Logger logger = Logger.getLogger(MediosMagneticos.class);
	...
	public void process() {
		logger.info(".. PROCESANDO BATCH MEDIOS MAGNETICOS ..");
...
//		String path_processBSCIVA = this.getPros().getProperty("fileProccessBSCSIVA");
		logger.info("path_process: " + path_process);
		logger.info("path_processBSC: " + path_processBSC);
//		logger.info("path_processBSCIVA: " + path_processBSCIVA);
					...
					try {
						...
						if (!FileUtil.fileExist(fileNameCopy)) {
							if (FileUtil.copy(fileNameFullPath, fileNameCopy)) {
								...
								// PREFIJO TXT Y PGP
								String fileOuput;
//								if(fileProcess.getName().startsWith(this.getPros().getProperty("fileIVA").trim())) {
//									fileOuput = this.getPros()
//											.getProperty("path").trim()
//											+ path_processBSCIVA
//											+ renameFile(fileName);
//								}else {
...
//								}
						...
						logger.error(" ERRROR en el proceso de Medios Magneticos  : "
								+ e.getMessage());
						String obervacion = "ERRROR en el proceso de Medios Magneticos: "
								+ e.getMessage();
						registrar_auditoriaV2(fileName, obervacion,uid);
					}
				}
