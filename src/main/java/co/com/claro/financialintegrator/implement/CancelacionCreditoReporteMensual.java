...
public class CancelacionCreditoReporteMensual extends GenericProccess {

	private Logger logger = Logger.getLogger(CancelacionCreditoReporteMensual.class);

	@Override
	public void process() {
		try{
			logger.info(".. iniciando proceso de Reporte mensual de Cambio Tasas Interes ..");
        ...
        String uid = uidResponse.getUid();
			...
			String pathFileRepoMensual = this.getPros().getProperty("pathReporte").trim();
			//Se crea directorio de proceso
			try {
				FileUtil.createDirectory( pathFile);
				FileUtil.createDirectory( pathFile + pathFileRepoMensual);
			} catch (FinancialIntegratorException e) {
				logger.error("Error creando directorio para processar archivo de ASCARD "
						+ e.getMessage());
			}
			
			String pathFileName = pathFile + pathFileRepoMensual + File.separator + name;
			Boolean execute = executeProd(this.configurationFileReporteMensual(pathFileName),uid);
			if (execute){
//				try {
//					String observacion="Archivo Procesado Exitosamente";
//					registrar_auditoria(pathFileName, observacion);
//				} catch (FinancialIntegratorException e) {
//					logger.error("Error copiando archivo " + e.getMessage(), e);
//					registrar_auditoria(pathFileName, "Error copiando archivo " + e.getMessage());
//				}
				logger.info("Archivo Procesado Exitosamente ");
			} else {
				logger.error("Error execute File ");
//				registrar_auditoria(pathFileName, "Error ejecutando procedimiento ");
			}
		} catch(Exception ex) {
			logger.error("Error generando archivo de reporte mensual de Cambio Tasas Interes " + ex.getMessage(), ex);
		}
	}
	...
	/**
	 * Se ejecuta procedimiento de Reporte mensual de Cambio Tasas Interes
	 */
	private Boolean executeProd(FileConfiguration fileConfig,String uid) {
		...
		try {
			...
			if (this.getPros().containsKey("DatabaseDataSource")) {
				...
				String call = this.getPros().getProperty("callCambioTasasInteresReporteMensual").trim();
			...
			logger.error("Error Ejecutando Reporte mensual de Cambio Tasas Interes " + ex.getMessage(), ex);
		} finally {
	...
	/**
	 * Configuración de reporte mensual de Cambio Tasas Interes
	 * @param file
	 * @return
	 */
	public FileConfiguration configurationFileReporteMensual(String file) {
		...
		type.setLength(0);
		type.setSeparator(separtor);
		type.setName("no_identificacion");
		type.setTypeData(new ObjectType(String.class.getName(), ""));
		_types.add(type);
		//
		type = new Type();
