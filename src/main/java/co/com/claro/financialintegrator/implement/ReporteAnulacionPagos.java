...
public class ReporteAnulacionPagos extends GenericProccess {
	private Logger logger = Logger.getLogger(ReporteAnulacionPagos.class);
	...
	public void process() {
		...
		logger.info(".. PROCESANDO BATCH REPORTE ANULACION PAGOS ..");
		// TODO Auto-generated method stub
			...
			for (File fileProcess : fileProcessList) {
				...
				if (fileProcess != null) {
					...
					try {
						...
						logger.error(" ERRROR en el proceso de Metricas Ascard: "
								+ e.getMessage());
						String obervacion = "ERRROR en el proceso de Metricas Ascard: "
								+ e.getMessage();
						registrar_auditoria(fileName, obervacion,uid);
					}
				}
