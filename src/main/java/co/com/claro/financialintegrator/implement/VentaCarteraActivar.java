...
public class VentaCarteraActivar extends GenericProccess {

	private Logger logger = Logger.getLogger(VentaCarteraActivar.class);

	@Override
	public void process() {
		logger.info(" -- PROCESANDO VENTA CARTERA ACTIVAR --");
	...
	private void insertarCreditosCastigados(List<FileOuput> lines, String nombreArchivo,String uid) {
		...
		try {
			...
			call = this.getPros().getProperty("callVentaCarteraActivar").trim();
		...
		for (FileOuput line : lines) {
			...
			try {
				...
				logger.info("NombreComprador: " + line.getType("NombreComprador").getValueString().trim());
				logger.info("TelefonoComprador: " + line.getType("TelefonoComprador").getValueString().trim());
				logger.info("DireccionComprador: " + line.getType("DireccionComprador").getValueString().trim());
				logger.info("IdentificacionComprador: " + line.getType("IdentificacionComprador").getValueString().trim());

				_database.setCall(call);

				List<Object> input = new ArrayList<Object>();
				input.add(line.getType("NumeroCredito").getValueString().trim());
				input.add(line.getType("NombreComprador").getValueString().trim());
				input.add(line.getType("TelefonoComprador").getValueString().trim());
				input.add(line.getType("DireccionComprador").getValueString().trim());
				input.add(line.getType("IdentificacionComprador").getValueString().trim());
				...
				if (cs != null) {
					logger.info("Call : " + this.getPros().getProperty("callVentaCarteraActivar").trim()
							+ " - P_RESULTADO : " + cs.getString(8)
							+ " - P_DESCRIPCION : " + cs.getString(9));
				}
	...
	private FileConfiguration configurationFileCreditosCastigadosVendidos(String file) {
		...
		type.setLength(0);
		type.setSeparator("|");
		type.setName("TelefonoComprador");
		type.setTypeData(new ObjectType(String.class.getName(), ""));
		_types.add(type);
		//
		type = new Type();
		type.setLength(0);
		type.setSeparator("|");
		type.setName("DireccionComprador");
		type.setTypeData(new ObjectType(String.class.getName(), ""));
		_types.add(type);
		//
		type = new Type();
		type.setLength(0);
		type.setSeparator("|");
		type.setName("IdentificacionComprador");
		type.setTypeData(new ObjectType(String.class.getName(), ""));
		_types.add(type);
