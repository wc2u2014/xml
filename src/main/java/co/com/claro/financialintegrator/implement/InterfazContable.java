...
/**
 * IF37: Batch que toma una archivo de Contabilidad de Ascard y lo desencripta y
 * renombre y lo pone en el FTP de BSCS
 * 
 * @author Oracle
 *
 */
public class InterfazContable extends GenericProccess {
	private Logger logger = Logger.getLogger(InterfazContable.class);

	/**
	 * Renombrar archivo de contabilidad para entregar a BSCS
	 * 
	 * @param fileName
	 *            archivo para renombar
	 * @return
	 */
	public String renameFile(String fileName) {
		...
		return fileName + this.getPros().getProperty("fileOutputExt");
	}
	...
	@Override
	public void process() {
		logger.info(".. PROCESANDO BATCH INTERFAZ CONTABLE ..");
		...
		try {
			...
			// logger.info("ExtfileProcess: "+this.getPros().getProperty("ExtfileProcess"));
			...
			for (File fileProcess : fileProcessList) {
				if (fileProcess != null) {
					...
					try {
						...
						logger.error(" ERROR en el proceso de Interfaz contable  : "
								+ e.getMessage());
						String obervacion = "ERRROR en el proceso de Interfaz contable: "
								+ e.getMessage();
						registrar_auditoria(fileName, obervacion,uid);
					}
				}
