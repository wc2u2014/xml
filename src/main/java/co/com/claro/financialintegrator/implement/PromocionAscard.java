...
public class PromocionAscard extends GenericProccess {
	private Logger logger = Logger.getLogger(PromocionAscard.class);
	...
	public void process() {
		...
		logger.info(".. PROCESANDO BATCH PROMOCION ASCARD ..");
		// TODO Auto-generated method stub
			...
			for (File fileProcess : fileProcessList) {
				...
				if (fileProcess != null) {
					...
					try {
						...
						if (!FileUtil.fileExist(fileNameCopy)) {
							if (FileUtil.copy(fileNameFullPath, fileNameCopy)) {
								...
								try {
									...
									copyControlRecaudo(renameFile(fileName), fileOuput,uid);
									try{
						...
						logger.error(" ERRROR en el proceso de Promocion Ascard  : "
								+ e.getMessage());
						String obervacion = "ERRROR en el proceso de Promocion Ascard: "
								+ e.getMessage();
						registrar_auditoriaV2(fileName, obervacion,uid);
					}
				}
