...
public class CreditoDigital extends GenericProccess {
	private Logger logger = Logger.getLogger(CreditoDigital.class);
	...
	public void process() {
		...
		logger.info(".. PROCESANDO BATCH CREDITO DIGITAL ..");
		// TODO Auto-generated method stub
			...
			for (File fileProcess : fileProcessList) {
				...
				if (fileProcess != null) {
					...
					try {
						...
						logger.error(" ERRROR en el proceso de Credito Digital: "
								+ e.getMessage());
						String obervacion = "ERRROR en el proceso de Credito Digital: "
								+ e.getMessage();
						registrar_auditoria(fileName, obervacion,uid);
					}
				}
