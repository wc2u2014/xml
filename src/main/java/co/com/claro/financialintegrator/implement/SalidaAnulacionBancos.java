...
public class SalidaAnulacionBancos extends GenericProccess {
	private Logger logger = Logger.getLogger(SalidaAnulacionBancos.class);
	...
	private List<FileOuput> processFile(String fileName, String path,String uid) {
		...
		if (fileName.startsWith(this.getPros().getProperty(
			...
			//
			try {
				list = FileUtil.readFile(TemplateSalidaAnulacionBancos
						.configurationSalidaAnulacionBancos(path));

				return processFileSalidaAnulacionBancos(list, fileName,uid);
	 ...
	 */
	private List<FileOuput> processFileSalidaAnulacionBancos(
		...
		for (FileOuput _line : lineDatos) {
			try {
				...
				if (_line.getHeader() == null) {
					...
					if (!_line.getType(TemplateSalidaAnulacionBancos.VALORPAGO)
							.getValueString().equals("codCorrecto")) {

						// Valor
						String VALOR = _line.getType(
								TemplateSalidaAnulacionBancos.VALORPAGO)
								.getValueString();

						String valorFormat = NumberUtils
								.formatValueClaro(VALOR);
						_line.getType(TemplateSalidaAnulacionBancos.VALORPAGO)
								.setSeparator("|");
						_line.getType(TemplateSalidaAnulacionBancos.VALORPAGO)
						...
						String REFERENCIA_PAGO = _line.getType(
								TemplateSalidaAnulacionBancos.REFERENCIA_PAGO)
								.getValueString();
						REFERENCIA_PAGO = String.valueOf(Long
								.parseLong(REFERENCIA_PAGO));
						// banco
						String banco = _line.getType(
								TemplateSalidaAnulacionBancos.CODMTV)
								.getValueString();
						String trx = _line.getType(
								TemplateSalidaAnulacionBancos.CODTRA)
								.getValueString();
						...
						// Fecha Ajustes
						_line.getType(TemplateSalidaAnulacionBancos.FECHAAJSUTE)
								.setSeparator("|");
						// CodMotivoPago
						_line.getType(TemplateSalidaAnulacionBancos.CODMTV)
								.setSeparator("|");
						_line.getType(TemplateSalidaAnulacionBancos.CODMTV)
						...
						_line.getType(TemplateSalidaAnulacionBancos.CODMTV)
								.setValue(banco);
						// CERCOD
						_line.getType(TemplateSalidaAnulacionBancos.CERCOD)
								.setSeparator("|");
						// CERDES
						_line.getType(TemplateSalidaAnulacionBancos.CERDES)
								.setSeparator("|");
						// CODTRA
						_line.getType(TemplateSalidaAnulacionBancos.CODTRA)
								.setSeparator("");						
						//
						List<Type> _typesNew = new ArrayList<Type>();
						_typesNew
								.add(_line
										.getType(TemplateSalidaAnulacionBancos.VALORPAGO));
						_typesNew
								.add(_line
										.getType(TemplateSalidaAnulacionBancos.FECHAAJSUTE));
						_typesNew.add(_line
								.getType(TemplateSalidaAnulacionBancos.CERCOD));
						_typesNew.add(_line
								.getType(TemplateSalidaAnulacionBancos.CERDES));
						_typesNew.add(_line
								.getType(TemplateSalidaAnulacionBancos.CODMTV));
						_typesNew
								.add(_line
										.getType(TemplateSalidaAnulacionBancos.REFERENCIA_PAGO));
						_typesNew
						.add(_line
								.getType(TemplateSalidaAnulacionBancos.CODTRA));
	 ...
	 * @return
	 */
	private Boolean creaFileSalidaAnulacionBancos(List<FileOuput> lineFileCreate) {
		...
		try {
			...
			if (FileUtil.createFile(fileName, lineFileCreate,
					new ArrayList<Type>(), TemplateSalidaAnulacionBancos
							.typesTemplateSalidaAnulacionBancosClaro())) {
	...
	@Override
	public void process() {
		logger.info(".. PROCESANDO BATCH SALIDA ANULACION BANCOS ..");
		...
		try {
			...
			if (linesConsolidado.size() > 0) {
				this.creaFileSalidaAnulacionBancos(linesConsolidado);
			}
