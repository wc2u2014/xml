...
public class CancelacionCreditoReporteDiario extends GenericProccess {

	private Logger logger = Logger.getLogger(CancelacionCreditoReporteDiario.class);
	...
	public void process() {
        ...
        String uid = uidResponse.getUid();
			logger.info(".. iniciando proceso de Reporte diario de Cambio Tasas Interes ..");
...
//				registrar_auditoria(pathFileName, "Error ejecutando procedimiento ");
			...
			logger.error("Error generando archivo de reporte diario de Cambio Tasas Interes " + ex.getMessage(), ex);
		}
	}
	...
	/**
	 * Se ejecuta procedimiento de Reporte diario de Cambio Tasas Interes
	 */
	private Boolean executeProd(FileConfiguration fileConfig,String uid) {
		...
		try {
			...
			if (this.getPros().containsKey("DatabaseDataSource")) {
				...
				String call = this.getPros().getProperty("callCambioTasasInteresReporteDiario").trim();
	...
	/**
	 * Configuración de reporte diario de Cambio Tasas Interes
	 * @param file
	 * @return
	 */
