...
public class CuotasGratis extends GenericProccess {
	private Logger logger = Logger.getLogger(CuotasGratis.class);
	...
	public void process() {
		...
		logger.info(".. PROCESANDO BATCH ARCHIVO CUOTAS GRATIS..");
		// TODO Auto-generated method stub
			...
			for (File fileProcess : fileProcessList) {
				...
				if (fileProcess != null) {
					...
					try {
						...
						logger.error(" ERRROR en el proceso de Cuotas Gratis  : "
								+ e.getMessage());
						String obervacion = "ERRROR en el proceso de Cuotas Gratis: "
								+ e.getMessage();
						registrar_auditoriaV2(fileName, obervacion,uid);
					}
				}
