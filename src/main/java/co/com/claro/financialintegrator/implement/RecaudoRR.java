 ...
 * @author freddylemus
 */
public class RecaudoRR extends GenericProccess {

	private Logger logger = Logger.getLogger(RecaudoRR.class);
	...
	/**
	 * Verifica la copia del archivo para el proceso de auditoria de los
	 * controles de recaudo
	 * 
	 * @param pathFile
	 *            ruta del archivo
	 * @param pathCopyFile
	 ...
	 */
	private void CopyExternalFiles(String name, String pathFile,
			String pathCopyFile, String fileNameCopyControlRecaudosBancosRR) {
		try {			
			if (isBancoRR(name)) {
				CopyExternalFiles(pathFile, fileNameCopyControlRecaudosBancosRR);
			} else {
				CopyExternalFiles(pathFile, pathCopyFile);
			}
		} catch (Exception e) {
			logger.error(
					"Error copiando archivos de Copia de Control Recaudos "
	...
	private void registrar_control_recaudo(String path, String fileName, String uid) {
		...
		try {
			if(isBancoRR(fileName)){
				...
				//Se obtiene información de recaudo sicacom rr
				FileConfiguration fc = new TemplateRecaudoSicacom()
						.configurationRecaudoSicacomUG(path);
				...
				cantidadRegistro = footer.getType("numero registro")
						.getValueString().trim();
				sumatoria = (BigDecimal) footer.getType("valor archivo")
						.getValue();
			}
	...
	public void process() {
		...
		String path_processCopyBloqueo = this.getPros().getProperty(
				"fileProccessCopyBloqueo");
		String path_processCopyControlRecaudos = this.getPros().getProperty(
				"fileProccessCopyControlRecaudos");
		String path_processCopyControlRecaudosBancosRR = this.getPros()
				.getProperty("fileProccessCopyControlRecaudos_Bancos");
		String path_processCopyAvisosPago = this.getPros().getProperty(
				"fileProccessCopyAvisosPago");
		...
		for (File fileProcess : fileProcessList) {
			...
			String fileNameCopyControlRecaudos = path_processCopyControlRecaudos
					+ fileName;
			...
			try {
				...
				if (!FileUtil.fileExist(fileNameCopy)) {
					if (FileUtil.copy(fileNameFullPath, fileNameCopy)) {
						...
						CopyBloqueoTerminal(fileNameFullPath,
								fileNameCopyBloqueo,uid);
						// Proceso encripcion
						...
						try {
							...
							CopyExternalFiles(fileName, fileNameFullPath,
									fileNameCopyControlRecaudos,
									fileNameCopyControlRecaudosBancosRR);
							// Se Copia Avisos Pago
							CopyExternalFiles(fileNameFullPath,
									fileNameAvisosPago);
