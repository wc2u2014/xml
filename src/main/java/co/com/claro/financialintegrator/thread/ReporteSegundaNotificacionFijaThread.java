...
public class ReporteSegundaNotificacionFijaThread implements Job{
	private Logger logger = Logger.getLogger(ReporteSegundaNotificacionFijaThread.class);
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		String fileOutputFechaReporteSegundaNotFija = map.getString("fileOutputFechaReporteSegundaNotificacion");
		String DatabaseDataSourceIntegrador = map.getString("DatabaseDataSourceIntegrador");
		String callSegundaNotFija = map.getString("callSegundaNotFija");
		...
		String prefijoReporteSegundaNotFija = map.getString("prefijoReporteSegundaNotFija");
		...
		String callActualizarSegundaNotFija = map.getString("callActualizarSegundaNotFija");		
		
		
		
		processreporteSegundaFacturaFija(fileOutputFechaReporteSegundaNotFija, DatabaseDataSourceIntegrador,callSegundaNotFija, 
				WSLAuditoriaBatchAddress,WSLAuditoriaBatchTimeOut,BatchName, path, fileOutputReporteSegundaNot, 
				prefijoReporteSegundaNotFija, fileProccess, processOutputReporteSegundaNot,callActualizarSegundaNotFija, uid);
		
	}
	
	public void processreporteSegundaFacturaFija(String fileOutputFechaReporteSegundaNotFija,String DatabaseDataSourceIntegrador, 
												String callSegundaNotFija, String wSLAuditoriaBatchAddress,
												String wSLAuditoriaBatchTimeOut, String batchName, String path, String fileOutputReporteSegundaNot,
												String prefijoReporteSegundaNotFija, String fileProccess, 
												String processOutputReporteSegundaNot, String callActualizarSegundaNotFija,String uid){
		OracleCallableStatement cs = null;
		Database _database = null;

		try {
			_database = Database.getSingletonInstance(DatabaseDataSourceIntegrador, null,uid);
			_database.setCall(callSegundaNotFija);
			...
			if (cs != null) {
				...
				if("TRUE".equals(result)){
					...
					if(rs.next()){
						String nombreArchivo = String.format(prefijoReporteSegundaNotFija,new Date());
						FileWriter fstream = new FileWriter(prefixName+nombreArchivo);
						BufferedWriter out = new BufferedWriter(fstream);
						logger.info(prefixName +"_"+ fileOutputFechaReporteSegundaNotFija+".txt");
				...
				logger.info("Tiempo de escritura"+estimatedTime);
					...
					registrar_auditoriaV2("REPORTE SEGUNDA NOT FIJA", "No se pudo ejecutar el reporte.",wSLAuditoriaBatchAddress, wSLAuditoriaBatchTimeOut, batchName);
				}
				
			}
			
			this.executeCall(callActualizarSegundaNotFija, null,null,DatabaseDataSourceIntegrador,uid);
		
		} catch (Exception e) {
			logger.error("Error generando reporte",e);
			registrar_auditoriaV2("REPORTE SEGUNDA NOT FIJA", "Error generando reporte",wSLAuditoriaBatchAddress, wSLAuditoriaBatchTimeOut, batchName);
		} finally {
