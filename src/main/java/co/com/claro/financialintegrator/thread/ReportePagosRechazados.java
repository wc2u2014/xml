...
public class ReportePagosRechazados implements Runnable {
	private Logger logger = Logger.getLogger(ReportePagosRechazados.class);
	//
	private BigDecimal ID_PAGOS_RECHAZADOS_GB;
	private String IMEI;
	private BigDecimal NRO_PRODUCTO;
	private java.sql.Date FECHA_RECHAZO;
	...
	//
	public ReportePagosRechazados(BigDecimal iD_PAGOS_RECHAZADOS_GB,
			String iMEI, BigDecimal nRO_PRODUCTO, Date fECHA_RECHAZO,
			String detalle, Connection integreadorConnection,
			Connection motorConnection, String callSourceMotorBloqueo,
			String callSourceIntegrador) {
		super();
		ID_PAGOS_RECHAZADOS_GB = iD_PAGOS_RECHAZADOS_GB;
		IMEI = iMEI;
		...
		try{
			...
			if (csMotorBloqueo!=null){
				...
				try {
					...
					parameterIntegrador.add(ID_PAGOS_RECHAZADOS_GB);
					...
					if(csIntegrador!=null){
						logger.info("ID_PAGOS_RECHAZADOS_GB:"+ ID_PAGOS_RECHAZADOS_GB +",Repuesta del integrador "+csIntegrador.getString(parameterIntegrador.size()+1));
					}
