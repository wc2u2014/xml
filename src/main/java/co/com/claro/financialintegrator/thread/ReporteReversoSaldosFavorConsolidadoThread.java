...
public class ReporteReversoSaldosFavorConsolidadoThread implements Job{
	private Logger logger = Logger.getLogger(ReporteReversoSaldosFavorConsolidadoThread.class);
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		String fileOutputFechaReversoSaldosFavorConsolidado = map.getString("fileOutputFechaReversoSaldosFavorConsolidado");
		String DatabaseDataSourceIntegrador = map.getString("DatabaseDataSourceIntegrador");
		String callReversoSaldosFavorConsolidado = map.getString("callReversoSaldosFavorConsolidado");
		...
		String fileOutputReporteReversoSaldosFavorConsolidado = map.getString("fileOutputReporteReversoSaldosFavorConsolidado");
		String prefijoReporteReversoSaldosFavorConsolidado = map.getString("prefijoReporteReversoSaldosFavorConsolidado");
		String fileProccess = map.getString("fileProccess");
		String processOutputReporteReversoSaldosFavorConsolidado = map.getString("processOutputReporteReversoSaldosFavorConsolidado");
		String pathBSCS = map.getString("pathBSCS");
		
		
		processreporteReversoSaldosFavorConsolidado(fileOutputFechaReversoSaldosFavorConsolidado, DatabaseDataSourceIntegrador,callReversoSaldosFavorConsolidado, 
				BSCSDataSource,WSLAuditoriaBatchAddress,WSLAuditoriaBatchTimeOut,BatchName,path,
				fileOutputReporteReversoSaldosFavorConsolidado,prefijoReporteReversoSaldosFavorConsolidado,fileProccess,processOutputReporteReversoSaldosFavorConsolidado,pathBSCS,uid);
		
	}
	
	public void processreporteReversoSaldosFavorConsolidado(String fileOutputFechaReversoSaldosFavorConsolidado,String DatabaseDataSourceIntegrador, 
												String callReversoSaldosFavorConsolidado, String bSCSDataSource, 
												String wSLAuditoriaBatchAddress, String wSLAuditoriaBatchTimeOut, String batchName,
												String path,String fileOutputReporteReversoSaldosFavorConsolidado,String prefijoReporteReversoSaldosFavorConsolidado,
												String fileProccess, String processOutputReporteReversoSaldosFavorConsolidado,String pathBSCS,String uid){
		...
		try {
			...
			FileUtil.createDirectory(path + fileOutputReporteReversoSaldosFavorConsolidado);
			FileUtil.createDirectory(path + processOutputReporteReversoSaldosFavorConsolidado);
		...
		try {
			...
			String prefixName=path.trim()+fileOutputReporteReversoSaldosFavorConsolidado.trim()+File.separator;
			String procesName=path.trim()+processOutputReporteReversoSaldosFavorConsolidado.trim()+File.separator;
			_database = Database.getSingletonInstance(DatabaseDataSourceIntegrador, null,uid);
			_database.setCall(callReversoSaldosFavorConsolidado);
				...
				cs = _database.executeCallOutputs(_database.getConn(uid),
						output, input,uid);	
				logger.info("Ejecuta reporte callReversoSaldosFavorConsolidado) "+cs);
				if (cs != null) {
					...
					if("TRUE".equals(result)){
						...
						if(rs.next()){
							String nombreArchivo = String.format(prefijoReporteReversoSaldosFavorConsolidado,new Date());
					...
					logger.info("Tiempo de escritura"+estimatedTime);
						...
						logger.info("No se pudo ejecutar el reporte del dia "+prefijoReporteReversoSaldosFavorConsolidado);
						registrar_auditoriaV2("REPORTE SALDOS FAVOR Reverso", "No se pudo ejecutar el reporte del dia "+prefijoReporteReversoSaldosFavorConsolidado,wSLAuditoriaBatchAddress, wSLAuditoriaBatchTimeOut, batchName);
					}
			...
			registrar_auditoriaV2("REPORTE SALDOS FAVOR Consolidado", "Error generando reporte",wSLAuditoriaBatchAddress, wSLAuditoriaBatchTimeOut, batchName);
		} finally {
